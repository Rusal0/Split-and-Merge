...
# User Input for Download Path
download_path = st.text_input("Enter download path (e.g., /home/user/downloads):")
...
if option == 'Split Excel by Sheets':
  ...
  if uploaded_file is not None:
    ...
    # Save the result to the specified download path
    with open(f"{download_path}/split_sheets.zip", "wb") as f:
      f.write(split_result.getbuffer())
    st.success(f"Split files saved to: {download_path}/split_sheets.zip")
...
# Merge Excel Files
  ...
  if uploaded_files:
    ...
    # Save the result to the specified download path
    with open(f"{download_path}/merged_file.xlsx", "wb") as f:
      f.write(merged_result.getbuffer())
    st.success(f"Merged file saved to: {download_path}/merged_file.xlsx")
